 ...
 */ 
var _SVC_INSURANCE = {
	makeInsuranceGroup : function (planNum) {
		// 서비스 요청
...
var C_INSURANCE = function(insuranceItem) {
	...
	// 현재까지 납입한 보험료
	this.getNowTotalPay = function() {
		return addComma(this.NOW_TOTAL_PAY);
	};
	// 납입 예정 보험료
	this.getRestTotalPay = function() {
		return addComma(this.REST_TOTAL_PAY);
	};
	// 총 납입 보험료
	this.getTotalPay = function() {
		return addComma(this.TOTAL_PAY);
	};
	// 피보험자 명수
...
var C_INSURANCE_GROUP = function(insuranceList) {
	...
	this.init = function(insuranceList) {
		...
		var allTotal	= {
			...
			"getSumNowTotalPay"		: function() {
				return addComma(this.NOW_TOTAL_PAY); 
			},
			"getSumRestTotalPay"	: function() {
				return addComma(this.REST_TOTAL_PAY); 
			},
			"getSumTotalPay"		: function() {
				return addComma(this.TOTAL_PAY); 
			}
		};
	...
	// 보장 기간별 담보 분석
	this.makeDamboMoneyAnalysisMap =  function(parm) {
		var insuredName 	= parm.insuredName;
		var damboGroupType 	= parm.damboGroupType;
